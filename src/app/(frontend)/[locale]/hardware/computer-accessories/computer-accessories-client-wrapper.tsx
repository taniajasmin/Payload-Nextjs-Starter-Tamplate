...
const jsonLd = {
  ...
  "@type": "ItemList",
  name: "Computer Accessories — Simal Technologies",
...
export function ComputerAccessoriesPageClient({ initialData, products }: Props) {
  ...
  const heroHeadline = hero?.headline || "Computer Accessories";
  const heroDescription = hero?.description || `Browse our complete range of <strong class="text-white">${products.length} authentic computer accessories</strong> from leading brands. USB-C hubs, docking stations, cables, surge protectors, and extension cords — sourced directly from authorized manufacturers with full warranty.`;
  const relatedLinks = (data.relatedLinks as string) || `Related Categories: <a href="/hardware/product-catalog" class="underline hover:text-neutral-900">Full Product Catalog</a> | <a href="/hardware/computer-components" class="underline hover:text-neutral-900">Computer Components</a> | <a href="/hardware/monitors" class="underline hover:text-neutral-900">Monitors</a> | <a href="/hardware/gaming" class="underline hover:text-neutral-900">Gaming</a> | <a href="/hardware/laptops" class="underline hover:text-neutral-900">Laptops</a>`;
    ...
    <div className="flex flex-col">
      ...
      <div className="relative bg-neutral-950 text-white">
        ...
        <div className="relative z-10 container-primary py-16 md:py-20">
          <h1 className="text-[length:var(--font-hero-heading)] font-bold tracking-tight">{heroHeadline}</h1>
          <p className="mt-4 text-[length:var(--font-body)] text-neutral-300 max-w-3xl" dangerouslySetInnerHTML={{ __html: heroDescription }} />
        </div>
      </div>
      <HardwareSubNav />
      <ProductCatalogClient
        products={products}
        pageTitle="Computer Accessories"
